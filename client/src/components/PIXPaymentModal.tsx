...
${pixPayment.pixCopyPaste}`;
    ...
    toast({
      ...
      description: "Clique novamente para enviar a 2ª mensagem com a chave PIX",
      duration: 4000,
    });

    // Atualizar o botão temporariamente para segunda mensagem
    const button = document.querySelector('[data-whatsapp-button]') as HTMLButtonElement;
    if (button) {
      ...
      button.innerHTML = `
        ...
        2ª Mensagem - Chave PIX
      `;
      
      // Função para enviar segunda mensagem
      const sendSecondMessage = () => {
        ...
        button.removeEventListener('click', sendSecondMessage);
        button.addEventListener('click', () => shareViaWhatsApp(pixPayment));
      };
      
      // Remover event listener atual e adicionar novo
      button.removeEventListener('click', () => shareViaWhatsApp(pixPayment));
      button.addEventListener('click', sendSecondMessage);
      
      // Restaurar automaticamente após 10 segundos
      setTimeout(() => {
        if (button.innerHTML.includes('2ª Mensagem')) {
          button.innerHTML = originalText;
          button.removeEventListener('click', sendSecondMessage);
          button.addEventListener('click', () => shareViaWhatsApp(pixPayment));
        }
